...
var OllamaSegQueryClassification = '## Query Types (classify first!)\n' +
  ...
  '   Step 2: get_full_document_content(file_identifier=path, section="SECTION_TITLE") → read that section\n' +
  '     (COPY the section title VERBATIM from the search result or the section-title list — never translate it. With no list, use the wording the doc itself uses: for a doc with English headers, English (e.g. "ADD COLUMN","ROLLUP"). section also works as an in-document search term: error-code lookup → section="2049". On a miss or a large doc, a section-title list returns — pick one and call again.)\n' +
  '     For a BROAD question ("X 기능 설명해줘", "X가 뭐야"): call WITHOUT section first — small docs return the FULL doc. Use section= for a specific operation, or after a section-title list returns. If you only received a title list, you MUST call again with one title FROM THAT LIST — NEVER answer from titles alone.\n' +
  ...
  '- The list_tables table is already shown to the user. Do NOT repeat it in your answer — say in one or two lines what is available and what to do next. Use the count the tool reported. The full-name list is for writing SQL/TQL only.\n' +
  '- Use table names EXACTLY as list_tables/describe_table returned them (e.g. MACHBASEDB.SYS.BITCOIN). Dropping the owner prefix resolves the name in the connected account schema, so a table owned by someone else fails with does not exist even when access was granted.\n' +
...
var OllamaSegSqlTools = '## SQL Tools\n' +
  ...
  'Stat phrase → SQL: 변동폭/range = MAX(VALUE)-MIN(VALUE); 주 단위/weekly = ROLLUP(\'week\',1,TIME); 일별=day, 시간별=hour; 표준편차/stddev = STDDEV(VALUE) via GROUP BY DATE_TRUNC. Do NOT split "주 단위"(weekly) into daily buckets.\n' +
  '**Anchor date is MANDATORY when the question names one.** If the question says "<날짜> 기준으로"/"기준일"/"starting from <date>" together with a week/month bucket, you MUST pass that date as the 4th ROLLUP argument: ROLLUP(\'week\',1,TIME,\'2024-01-01\'). DATE_TRUNC CANNOT honor an anchor date (it always snaps weeks to Monday) — using it there silently returns buckets the user did not ask for. Do NOT answer with a note explaining that the boundary differs from what was asked; use the anchor argument so it matches.\n' +
