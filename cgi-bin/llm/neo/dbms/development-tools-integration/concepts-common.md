...
나면 값을 문자열로 합치기 전에 Named Bind Parameter와
...
완전한 SQL 예제는
TRANSACTION 테이블의 트랜잭션을 참고합니다.

## Append API와 batch

입력 방식 선택과 결과 확인 항목은 데이터 입력과 반출에서,
클라이언트·테이블 타입별 Append gate는
SDK Append 지원표에서 다룹니다.
