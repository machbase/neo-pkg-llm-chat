...
| 섹션 | 설명 |
|------|------|
| 메타 테이블 사전 | M$SYS_TABLES, M$SYS_COLUMNS 등 스키마 메타 테이블 상세 |
| 가상 테이블 사전 | V$SESSION, V$STMT, V$PROPERTY 등 동적 뷰 상세 |
| TAG별 통계 뷰 | `V$<TABLE>_STAT`의 시간·거리축별 스키마와 조회 방법 |
| V$ROLLUP 사전 | Rollup 작업 상태 뷰 컬럼 상세 |
| V$STORAGE_MOUNT_* 사전 | 마운트된 백업 데이터베이스 뷰 컬럼 상세 |
| 전체 가상 테이블 레퍼런스 | 8.5 원본 가상 테이블 레퍼런스의 전체 항목 |
