...
MQTT v5의 사용자 속성은 각 메시지에 붙일 수 있는 사용자 정의 키-값 쌍입니다. 이전 버전(MQTT v3.1/v3.1.1)보다 유연하며 메시지에 추가 메타데이터를 담을 수 있습니다.

**참고: 사용자 정의 속성 없이 MQTT v5에서 MQTT v3.1 토픽 문법을 그대로 쓸 수도 있습니다.**
0
MQTT v5를 사용하면 토픽 문법이 단순히 `db/write/{table}`이 되며 다음 속성들을 지원합니다:
...
강조된 `header` `skip` 옵션은 첫 줄이 헤더임을 나타냅니다.
...
`method` 옵션의 기본값은 `insert`이므로 생략할 수 있습니다.
